...
def irradiation_extraterrestre_normale(n,cas =1):
    if cas == 2:
        B = (n-1)*360/365.25
        G = Gsc*(1.000110+0.034221*cosd(B)+0.00128*sind(B)+0.000719*cosd(2*B)+0.000077*sind(2*B))
    else:
...
def Calcul_Kta_moyen(phi,beta,gam,rhog,bo,b1,njt,Hm,KTm):
    ...
    for ih in range(0,nn):
        ...
        s3 = s3 + Kb*Ib*Rb
        s4 = s4 + Kd*Id*(1+cosd(beta))/2
        s5 = s5 + Kr*I*rhog*(1-cosd(beta))/2
        S = Kb*Ib*Rb + Kd*Id*(1+cosd(beta))/2 + Kr*I*rhog*(1-cosd(beta))/2
        s1 = s1 + S
        s2 = s2 + IT
    ta =  s1/s2
...
def Calcul_phi_bar(nj,phi,beta,gam,Hm,Itc,rhog,KTm,cas = 1):
