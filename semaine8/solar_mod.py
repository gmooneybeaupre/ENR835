...
def heure_angle(ome):
    if ome == 0:
        ...
        if(minu >= 60):
            minu = minu - 60
            hr = hr+1
    sol_tn = temps(heure = hr,minu = minu)
    return sol_tn
...
def  Calcul_Rb_mois(phi,n,beta,gam):
    ...
    ome1 = np.arccosd(-np.tand(phi)*np.tand(delt))
    s = np.sign(cosd(gam))
...
def  Calcul_Rb_mois_KTg(phi,n,beta,gam,KT):
    ...
    delt  = decl_solaire(n)
    omes = np.arccosd(-tand(phi)*tand(delt))
    ...
    ome1 = np.arccosd(come1)
    ome2 = np.arccosd(come2)
    omesr = min(omes,ome1)
    omess = min(omes,ome2)
...
def Calcul_tau(the1,KL,n2=1.526,n1 = 1,N=1):
    the2 = np.arcsind(n1*sind(the1)/n2)
    th12 = the1+the2
...
def Calcul_kta_nouv(beta,b0,b1):
    ...
    for i in range(0,360):
        for j in range(0,90):
            ...
            Incidence_Angle=np.arccosd(cosd(Zenith_Angle)*cosd(Slope_Collector)+sind(Zenith_Angle)*sind(Slope_Collector)*cosd(SolarAzimuth_Angle))
